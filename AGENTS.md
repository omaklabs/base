...
| `templates/components` | Shadcn-style UI components with typed Go APIs. Button, Card, Input, Textarea, SelectField, Badge, Flash, Label, ErrorText, FormField, NavItem, PageHeader, EmptyState, Pagination. All styling via inline Tailwind classes using design tokens. | Create CSS component classes. Use `@apply`. Always use the component functions. |
...
UI components live in `templates/components/` as templ files with inline Tailwind classes. They follow
the shadcn pattern — typed Go APIs, design token-based colors, two-level API (simple + full).

**Simple API** (most common — positional string arguments):
```go
@components.Button("primary") { Save }
@components.Button("destructive", "sm") { Delete }
@components.SubmitButton("primary") { Create Item }
@components.LinkButton("primary", "/items/new") { New Item }
@components.Card() { content }
@components.Card("sm") { compact content }
@components.Badge("success") { Active }
@components.Input("email", "Email address")
@components.FormField("Email", "email", "email", value, errMsg)
@components.NavItem("/items", isActive) { Items }
@components.PageHeader("Items") { @components.LinkButton("primary", "/items/new") { New } }
@components.EmptyState("No items yet.") { @icons.Inbox() }
@components.Label("name") { Full Name }
@components.ErrorText(errMsg)
```

**Full API** (for modifiers — loading, icon-only, full-width, disabled):
```go
@components.ButtonWith(components.ButtonProps{Variant: "primary", Loading: true, Type: "submit"}) { Saving... }
@components.InputWith(components.InputProps{Name: "email", Type: "email", HasError: true, Required: true})
@components.TextareaWith(components.TextareaProps{Name: "body", Rows: 8, HasError: true})
@components.SelectFieldWith(components.SelectFieldProps{Name: "role", Options: opts})
```
...
2. Simple API: `templ ComponentName(args...) { <html class={...}>{ children... }</html> }`
3. Full API: `templ ComponentNameWith(props ComponentNameProps) { ... }`
4. Use `cx()` helper to build class strings
5. Reference design tokens, not hardcoded colors
