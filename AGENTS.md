...
| `templates/components` | 40 shadcn-style UI components. Flat (26): Button, Card, Input, Textarea, SelectField, Badge, Flash, Label, ErrorText, FormField, SelectFormField, TextareaFormField, CheckboxField, RadioGroup, RadioFormField, Checkbox, NavItem, PageHeader, EmptyState, Pagination, Spinner, Alert, Skeleton, Progress, Breadcrumb, Separator, Typography (H1-H4, P, Lead, Muted), ThemeToggle. Compound Lit (14 sub-packages): dialog, alertdialog, dropdown, tabs, toast, tooltip, switchc, sheet, accordion, popover, combobox, sidebar, table, avatar. All use oklch design tokens, `--radius`, light+dark mode, `Attrs templ.Attributes` escape hatch. | Create CSS component classes. Use `@apply`. Always use the component functions. |
...
UI components live in `templates/components/` as templ files with inline Tailwind classes. They use **variadic props** — every component accepts zero or one props struct. Zero-arg gives sensible defaults; pass the struct for customization.

Every props struct has:
- Named fields for common options (Variant, Size, etc.)
- `Class string` — appended to computed classes (no override/merge)
- `Attrs templ.Attributes` — escape hatch for `hx-get`, `data-*`, `aria-*`, etc.

**Flat components** (`templates/components/`):
```go
...
@components.LinkButton("/items/new") { New Item }

// With props
@components.Button(components.ButtonProps{Variant: "destructive", Size: "sm"}) { Delete }
@components.Button(components.ButtonProps{Loading: true, Disabled: true}) { Saving... }
@components.Button(components.ButtonProps{
    Attrs: templ.Attributes{"hx-delete": "/items/1", "hx-confirm": "Sure?"},
}) { Delete via HTMX }
@components.LinkButton("#", components.ButtonProps{Variant: "outline"}) { Outline Link }
@components.Card(components.CardProps{Padding: "sm"}) { compact }
@components.Badge(components.BadgeProps{Variant: "success"}) { Active }
@components.Input(components.InputProps{Type: "email", Name: "email", Placeholder: "you@example.com"})
@components.Textarea(components.TextareaProps{Name: "body", Rows: 8})
@components.SelectField(components.SelectFieldProps{Name: "role", Options: opts})
...
**Compound components** (sub-packages under `templates/components/`):
```go
import "your-module/templates/components/dialog"
import "your-module/templates/components/dropdown"
import "your-module/templates/components/tabs"
import "your-module/templates/components/toast"
import "your-module/templates/components/table"
import "your-module/templates/components/avatar"
import "your-module/templates/components/tooltip"

// Dialog (Alpine.js x-data="dialog()")
@dialog.Dialog(dialog.Props{ID: "confirm"}) {
    @dialog.Trigger() { @components.Button() { Open } }
    @dialog.Content() {
        @dialog.Header() { <h3>Title</h3> }
        @dialog.Body() { <p>Content here</p> }
        @dialog.Footer() {
            @dialog.Close() { @components.Button(components.ButtonProps{Variant: "ghost"}) { Cancel } }
            @components.Button(components.ButtonProps{Variant: "destructive"}) { Confirm }
        }
    }
}

// Dropdown (Alpine.js x-data="dropdown()")
@dropdown.Dropdown() {
    @dropdown.Trigger() { @components.Button(components.ButtonProps{Variant: "outline"}) { Menu } }
    @dropdown.Content() {
...
// Tabs (Alpine.js x-data="tabs('tab1')")
@tabs.Tabs(tabs.Props{Default: "overview"}) {
    ...
    @tabs.Content(tabs.ContentProps{Value: "overview"}) { <p>Overview</p> }
    @tabs.Content(tabs.ContentProps{Value: "settings"}) { <p>Settings</p> }
}

// Toast — client-side (Alpine store) or server-side (HTMX OOB swap)
// Client: use @click={ toast.Show("success", "Saved!", 4000) }
// Server: render toast.Toast(toast.Props{Variant: "success"}) { Saved! } with HX-Retarget

// Table (pure HTML, no JS)
@table.Table() {
    @table.Header() {
        @table.Row() { @table.HeaderCell() { Name } @table.HeaderCell() { Email } }
    }
    @table.Body() {
        @table.Row() { @table.Cell() { Alice } @table.Cell() { alice@example.com } }
    }
}

// Avatar
@avatar.Avatar(avatar.Props{Initials: "AJ"})
@avatar.Avatar(avatar.Props{Src: "/img/user.jpg", Alt: "User"})
...
**Flat component** (single file in `templates/components/`):
1. Create `templates/components/<name>.templ`
2. Define `<Name>Props` struct with `Class string` and `Attrs templ.Attributes`
3. Add `<name>Defaults(props []<Name>Props) <Name>Props` helper
4. `templ <Name>(props ...<Name>Props) { ... }`
