...
| `templates/components` | Shadcn-style UI components with variadic props APIs. Flat: Button, Card, Input, Textarea, SelectField, Badge, Flash, Label, ErrorText, FormField, NavItem, PageHeader, EmptyState, Pagination. Compound (sub-packages): dialog, dropdown, tabs, toast, table, avatar, tooltip. All support `Attrs templ.Attributes` for HTMX/data attributes. | Create CSS component classes. Use `@apply`. Always use the component functions. |
